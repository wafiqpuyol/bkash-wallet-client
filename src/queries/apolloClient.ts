...
const httpUrl: string = process.env.NEXT_PUBLIC_NODE_ENV === 'development' ? 'http://localhost:3500/graphql' : `${process.env.NEXT_PUBLIC_HTTP_SERVER_URL}/graphql`;
const wsUrl: string = process.env.NEXT_PUBLIC_NODE_ENV === 'development' ? 'ws://localhost:3500/graphql' : `${process.env.NEXT_PUBLIC_WS_SERVER_URL}/graphql`;
...
const initPersistorCache = async (): Promise<void> => {
    apolloPersistor = new CachePersistor({
        ...
        debug: false,
        trigger: 'write' // if any new data is written to the cache, trigger a write
    });
