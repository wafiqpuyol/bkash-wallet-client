...
import { IHeartbeat, IStatusPage } from '@/interfaces/monitor';

export const StatusTable: React.FC<IStatusPage> = ({ limit, heartBeats }) => {
    const dateFormat = (timestamp: string): string => {
        return dayjs(new Date((timestamp))).format('YYYY-MM-DD HH:mm:ss A');
    };
