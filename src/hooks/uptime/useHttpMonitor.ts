...
import { httpSchemaValidation } from '@/validation/http';
...
export const useHttpEdit = (monitorId: string): IUseUptime => {
    ...
    console.log("Signle monitor  ----->", monitorData);
    ...
    useEffect(() => {
        if (monitorData) {
            ...
            if (!monitors[0].contentType.startsWith('a')) {
                monitors[0].contentType = JSON.parse(JSON.stringify(`${monitors[0]?.contentType}`.replace().slice(1, -1)))
            }
            setMonitorInfo({
                id: monitors[0]?.id,
